...
transformdata_hotspot=function(data.trans,value,filter,infos)  #
{
  ...
  for(i in 1:nrow(infos))
   {
      ...
      data1$shape=as.numeric(shapes[i])

      data.trans2[[i]]<-data1
      dat.table[[i]]=as.data.frame(data1[data1$size%in%7,1:4])
      venn.list[[infos$Abbreviation[i]]]<-data1$SNP[which(data1$size==7)]
  }
  ...
  data.list=list(data.trans4,data2,venn.list)
  data.list
}
...
transformdata_hotspot_bc=function(data.trans,value,filter,infos,approach)  #
{
  data.trans3<-dat.table<-list();venn.list=list()
...
scatterPlot_hotspot_all <- function(dat1,file.name)
{
  BP<-ChrTheta<-coloring<-NULL

  dat1=dat1[order(dat1$what),] ## order necessary otherwise legend and coloring is wrong

  p<- plotly::plot_ly(data=dat1, x = ~Mbp, y = ~ChrTheta, color=~what, type = "scatter", mode="markers",marker=list(size=~size),hoverinfo="text", fill=~'',text=dat1$SNP, symbol=~shape,
                      ...
                      hovertemplate = paste("<b>Chromosome:</b> ", sprintf("%.0f", dat1$Chr),"<br>","<b>BTA:</b> %{text} <br>","<b>Theta:</b>", sprintf("%.3f", dat1$Theta),"<br>",
                                            "<b>Physical length (Mbp):</b>", sprintf("%.3f", dat1$Mbp),"<br><extra></extra>"),
                      height=1000)%>%
      plotly::layout(xaxis=list(title="Chromosome position (Mbp)"),yaxis=list(title="Chromosome",tickvals=c(1:29),ticktext=1:29)) %>%
...
scatterPlot_hotspot <- function(dat1,file.name)
{
  BP<-Theta<-coloring<-ord<-NULL

  dat1=dat1[order(dat1$what),] ## order necessary otherwise legend and coloring is wrong # hover not correct XXX

  p<- plotly::plot_ly(data=dat1, x = ~Mbp, y = ~Theta, color=~what, type = "scatter", mode="markers",marker=list(size=~size),hoverinfo="text", fill=~'',text=dat1$SNP, symbol=~shape,
                     colors=unique(dat1$coloring),
                     hovertemplate = paste("<b>BTA: %{text}</b><br>","Theta: %{y:.}<br>","Physical length (Mbp): %{x:.}<br>","<extra></extra>"))%>%
      plotly::layout(legend=list(title=list(text="Legend",font=list(size=15,family="Arial")),font=list(family="Arial",size=15)),
                     title=list(text=paste0("BTA ",unique(dat1$Chr)),font=18), xaxis=list(title="Chromosome position (Mbp)"),yaxis=list(title="Recombination rate")) %>%
      plotly::config(displayModeBar=TRUE,toImageButtonOptions= list(filename = file.name))
  p
}
